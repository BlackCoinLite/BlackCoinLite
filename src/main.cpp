...
#if defined(NDEBUG)
# error "BlackCoinLite cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "BlackCoinLite Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
	...
	//ICO Premine (3200000000)
	if (pindexBest->nHeight+1 <= 100) 
	{
		nSubsidy = 32000000 * COIN;
	}
	
	//TX fee for ICO transaction (0.00000500)
	if (pindexBest->nHeight+1 == 101) 
	{
		nSubsidy = 500;
	}
...
bool CBlock::AcceptBlock()
{
    ...
    if (GetBlockTime() <= pindexPrev->GetBlockTime() || FutureDrift(GetBlockTime()) < pindexPrev->GetBlockTime())
...
{
    RenameThread("blackcoinlite-loadblk");
